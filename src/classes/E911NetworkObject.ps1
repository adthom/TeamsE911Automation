class E911NetworkObject {
    ...
    hidden Init([PSCustomObject]$obj) {
        ...
        if ((![string]::IsNullOrEmpty($this._locationId) -and $this._location.Id.ToString() -ne $this._locationId) -or $this._location._hasChanged) {
    ...
    E911NetworkObject([bool]$online, [hashtable] $Hash) {
        $this.Type = $Hash['Type']
        $this.Identifier = $Hash['Identifier']
