class E911ModuleState {
    ...
    hidden static [Lazy[Collections.Generic.Dictionary[Collections.Generic.HashSet[string],string]]] $s_replacementdictionary = [Lazy[Collections.Generic.Dictionary[Collections.Generic.HashSet[string],string]]]::new(
        [Func[Collections.Generic.Dictionary[Collections.Generic.HashSet[string],string]]] { 
            $dict = [Collections.Generic.Dictionary[Collections.Generic.HashSet[string],string]]@{}
            foreach ($key in [E911ModuleState]::s_replacementHash.Keys) {
                $dict[[Collections.Generic.HashSet[string]]([string[]][E911ModuleState]::s_replacementHash[$key])] = $key
            }
            return $dict
        })

    hidden static [Collections.Generic.Dictionary[Collections.Generic.HashSet[string],string]] $AbbreviationLookup = [E911ModuleState]::s_replacementdictionary.Value
    ...
    static [Collections.Generic.List[string]] GetCleanAddressParts([string] $Address) {
        $Address = $Address.ToLowerInvariant()
        $AddressTokens = [Collections.Generic.List[string]][string[]]($Address -split '\b').Where({$_ -match '\w'}).ForEach({$_.Trim()})
        for ($i = 0; $i -lt $AddressTokens.Count; $i++) {
            ...
            if ([E911ModuleState]::Abbreviations.Contains($CurrentToken)) {
                ...
                if ($CurrentToken -match '^(n(orth)?|s(outh)?)(e(ast)?|w(est)?)$') {
    ...
    hidden static [System.Collections.Generic.Dictionary[string, E911Address]] $OnlineAddresses = @{}
    hidden static [System.Collections.Generic.Dictionary[string, E911Address]] $Addresses = @{}
    hidden static [System.Collections.Generic.Dictionary[string, E911Location]] $OnlineLocations = @{}
    hidden static [System.Collections.Generic.Dictionary[string, E911Location]] $Locations = @{}
    hidden static [System.Collections.Generic.Dictionary[string, E911NetworkObject]] $OnlineNetworkObjects = @{}
    hidden static [System.Collections.Generic.Dictionary[string, E911NetworkObject]] $NetworkObjects = @{}

    static [E911Address] GetOrCreateAddress([PSCustomObject] $obj, [bool] $ShouldValidate) {
        $Hash = [E911Address]::GetHash($obj)
        $Test = $null
        # if ([E911ModuleState]::Addresses.TryGetValue($Hash, [ref] $Test) -and [string]::IsNullOrEmpty($obj.CivicAddressId)) {
        ...
        #     return $Test
        # }
        # if ((!$New._isOnline -and $ShouldValidate)) {
    ...
    static [void] InitializeCaches([PSFunctionHost] $parent) {
        ...
        [LisObjectHelper]::LoadCache($currentProcess)
        $lisLocations = [LisLocation]::GetAll($true)
