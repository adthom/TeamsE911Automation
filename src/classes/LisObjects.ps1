class CachedLisObject {
    ...
    static [LisObject] GetFromCache([Type] $TypeInfo, [string] $identifier) {
        $Cache = [LisCache]::Cache[$TypeInfo]
        $Result = $null
    ...
    static [Collections.Generic.List[LisObject]] GetAllFromCache([Type] $TypeInfo) {
        $Cache = [LisCache]::Cache[$TypeInfo]
        if ($null -ne $Cache) {
    ...
    static [bool] IsMissing([Type] $TypeInfo, [string] $identifier) {
...
class LisCache {
    static [Collections.Generic.Dictionary[Type, Collections.Generic.Dictionary[string, CachedLisObject]]] $Cache = @{}
    static [Collections.Generic.Dictionary[Type, Collections.Generic.HashSet[string]]] $Missing = @{}
...
class LisObject {
    ...
    hidden static [Collections.Generic.List[LisObject]] _getAll([Type] $type, [Management.Automation.CommandInfo] $command) {
        return [LisObject]::_get($type, $command, '', @{})
    }
    hidden static [Collections.Generic.List[LisObject]] _get([Type] $type, [Management.Automation.CommandInfo] $command, [string] $identifier, [hashtable] $additionalParams) {
        ...
        if ($PointQuery) {
            if ([CachedLisObject]::IsMissing($type, $identifier)) { return @() }
            if ($additionalParams.Keys.Count -eq 0) {
                $Result = [CachedLisObject]::GetFromCache($type, $identifier)
            ...
            $Result = [CachedLisObject]::GetAllFromCache($type)
            if ($null -ne $Result) { return [LisObject[]]$Result }
        }
        try {
    ...
    # }
    hidden static [Reflection.PropertyInfo[]] _getPublicPwshClassProperties([Type] $type) {
        return [Reflection.PropertyInfo[]]$type.GetProperties('Instance,Public').Where({
            $_.CustomAttributes.Where({$_.AttributeType -eq [Management.Automation.HiddenAttribute]},'First',1).Count -eq 0})
    }
    [string] ToString() {
...
class LisNetworkObject : LisObject {
    ...
    [bool] IsOrphaned() {
        if (!$this._orphanedCheckDone) {
            $location = $this.GetLocation()
            $this._isOrphaned = $null -eq $location -or $location.IsOrphaned()
        }
        return $this._isOrphaned
    }
    static [Collections.Generic.List[LisNetworkObject]] GetAll() {
        return [LisNetworkObject]::GetAll($false)
    }
    static [Collections.Generic.List[LisNetworkObject]] GetAll([bool] $IncludeOrphaned) {
        $result = [Collections.Generic.List[LisNetworkObject]]@()
        $result.AddRange([LisPort[]][LisPort]::GetAll().Where({ $IncludeOrphaned -or !$_.IsOrphaned() }))
        $result.AddRange([LisSwitch[]][LisSwitch]::GetAll().Where({ $IncludeOrphaned -or !$_.IsOrphaned() }))
        $result.AddRange([LisSubnet[]][LisSubnet]::GetAll().Where({ $IncludeOrphaned -or !$_.IsOrphaned() }))
        $result.AddRange([LisWirelessAccessPoint[]][LisWirelessAccessPoint]::GetAll().Where({ $IncludeOrphaned -or !$_.IsOrphaned() }))
        return $result
    }
    [LisLocation] GetLocation() {
        return [LisLocation]::GetById($this.LocationId)
    }
    [LisCivicAddress] GetCivicAddress() {
        $location = $this.GetLocation()
        return [LisCivicAddress]::GetById($location.CivicAddressId)
    }
}
...
class LisAddressBase : LisObject {
    ...
    [Management.Automation.CommandInfo] GetItemCommand() { throw 'Not implemented' }
    [bool] IsInUse() {
        if (!$this._useCheckDone) {
            if ($this.NumberOfTelephoneNumbers -eq -1 -or $this.NumberOfVoiceUsers -eq -1) {
                $WithInfo = [LisAddressBase]::_get($this.Identifier(), $this.GetItemCommand(), $this.GetType(), $true)
            ...
            if (!$this._isInUse) {
                $this._isInUse = $this.GetAssociatedNetworkObjects().Count -gt 0
            }
    ...
    hidden static [Collections.Generic.List[LisAddressBase]] _get([string] $identifier, [Management.Automation.CommandInfo] $command, [Type] $type, [bool] $Populate) {
        $additionalParams = @{}
        if ($Populate) {
            $additionalParams['PopulateNumberOfTelephoneNumbers'] = $true
            $additionalParams['PopulateNumberOfVoiceUsers'] = $true
        }
        return [LisAddressBase[]][LisObject]::_get($type, $command, $identifier, $additionalParams)
    }

    [Collections.Generic.List[LisNetworkObject]] GetAssociatedNetworkObjects() { throw 'Must Override Base Method' }
...
class LisLocation : LisAddressBase {
    hidden static [Management.Automation.CommandInfo] $_getItemCommand = $null
    [Guid] $LocationId
    [string] $Location
    LisLocation() {}
    ...
    [Management.Automation.CommandInfo] GetItemCommand() {
    ...
    [bool] IsValid() {
        if (!$this._validDone) {
            ...
            $this._isValid = !$this.IsOrphaned() -and $address.IsValid()
    ...
    static [LisLocation] GetById([string] $LocationId, [bool] $Populate) {
        return [LisLocation]::Get($LocationId, $Populate)[0]
    }
    static [Collections.Generic.List[LisLocation]] GetAll() {
        return [LisLocation]::GetAll($false)
    }
    static [Collections.Generic.List[LisLocation]] GetAll([bool] $Populate) {
        return [LisLocation]::Get('', $Populate)
    }
    static [Collections.Generic.List[LisLocation]] Get([string] $LocationId, [bool] $Populate) {
        if ($null -eq [LisLocation]::_getItemCommand) {
            [LisLocation]::_getItemCommand = Get-Command -Name Get-CsOnlineLisLocation
        }
        return [LisLocation[]][LisAddressBase]::_get($LocationId, [LisLocation]::_getItemCommand, [LisLocation], $Populate)
    }
    [Collections.Generic.List[LisNetworkObject]] GetAssociatedNetworkObjects() {
        return [LisNetworkObject[]][LisNetworkObject]::GetAll().Where({ $_.LocationId -eq $this.LocationId })
    }
    hidden [string] $_hash
    [string] GetHash() {
    ...
    [LisCivicAddress] GetCivicAddress() {
        return [LisCivicAddress]::GetById($this.CivicAddressId)
    }
...
class LisCivicAddress : LisAddressBase {
    ...
    [bool] IsValid() {
        if (!$this._validDone) {
            $location = [LisLocation]::GetById($this.DefaultLocationId)
            ...
            if (!$this._isValid) {
                $AllLocations = [LisLocation]::GetAll().Where({ $_.CivicAddressId -eq $this.CivicAddressId })
    ...
    static [LisCivicAddress] GetById([string] $CivicAddressId, [bool] $Populate) {
        return [LisCivicAddress]::Get($CivicAddressId, $Populate)[0]
    }
    static [Collections.Generic.List[LisCivicAddress]] GetAll() {
        return [LisCivicAddress]::GetAll($false)
    }
    static [Collections.Generic.List[LisCivicAddress]] GetAll([bool] $Populate) {
        return [LisCivicAddress]::Get('', $Populate)
    }
    static [Collections.Generic.List[LisCivicAddress]] Get([string] $CivicAddressId, [bool] $Populate) {
        if ($null -eq [LisCivicAddress]::_getItemCommand) {
            [LisCivicAddress]::_getItemCommand = Get-Command -Name Get-CsOnlineLisCivicAddress
        }
        return [LisCivicAddress[]][LisAddressBase]::_get($CivicAddressId, [LisCivicAddress]::_getItemCommand, [LisCivicAddress], $Populate)
    }
    [Collections.Generic.List[LisLocation]] GetAssociatedLocations() {
        return [LisLocation[]][LisLocation]::GetAll().Where({ $_.CivicAddressId -eq $this.CivicAddressId })
    }
    [Collections.Generic.List[LisNetworkObject]] GetAssociatedNetworkObjects() {
        $All = [LisNetworkObject]::GetAll()
        $Locations = [Collections.Generic.HashSet[Guid]][Guid[]]$this.GetAssociatedLocations().LocationId
        return [LisNetworkObject[]]$All.Where({ $_.LocationId -in $Locations })
    }
    hidden [string] $_hash
    [string] GetHash() {
        if ([string]::IsNullOrEmpty($this._hash)) {
...
class LisPort : LisNetworkObject {
    ...
    static [Collections.Generic.List[LisPort]] GetAll() {
        if ($null -eq [LisPort]::_getItemCommand) {
            [LisPort]::_getItemCommand = Get-Command -Name Get-CsOnlineLisPort
        }
        return [LisPort[]][LisObject]::_getAll([LisPort], [LisPort]::_getItemCommand)
    }
    static [LisPort] Get([string] $identifier) {
        if ($null -eq [LisPort]::_getItemCommand) {
            [LisPort]::_getItemCommand = Get-Command -Name Get-CsOnlineLisPort
        }
        return [LisPort][LisObject]::_get([LisPort], [LisPort]::_getItemCommand, $identifier, @{})
    }
...
class LisSwitch : LisNetworkObject {
    ...
    static [Collections.Generic.List[LisSwitch]] GetAll() {
        if ($null -eq [LisSwitch]::_getItemCommand) {
            [LisSwitch]::_getItemCommand = Get-Command -Name Get-CsOnlineLisSwitch
        }
        return [LisSwitch[]][LisObject]::_getAll([LisSwitch], [LisSwitch]::_getItemCommand)
    }
    static [LisSwitch] Get([string] $identifier) {
        if ($null -eq [LisSwitch]::_getItemCommand) {
            [LisSwitch]::_getItemCommand = Get-Command -Name Get-CsOnlineLisSwitch
        }
        return [LisSwitch][LisObject]::_get([LisSwitch], [LisSwitch]::_getItemCommand, $identifier, @{})
    }
}
...
class LisSubnet : LisNetworkObject {
    ...
    static [Collections.Generic.List[LisSubnet]] GetAll() {
        if ($null -eq [LisSubnet]::_getItemCommand) { [LisSubnet]::_getItemCommand = Get-Command -Name Get-CsOnlineLisSubnet }
        return [LisSubnet[]][LisObject]::_getAll([LisSubnet], [LisSubnet]::_getItemCommand)
    }
    static [LisSubnet] Get([string] $identifier) {
        if ($null -eq [LisSubnet]::_getItemCommand) { [LisSubnet]::_getItemCommand = Get-Command -Name Get-CsOnlineLisSubnet }
        return [LisSubnet][LisObject]::_get([LisSubnet], [LisSubnet]::_getItemCommand, $identifier, @{})
    }
}
...
class LisWirelessAccessPoint : LisNetworkObject {
    ...
    static [Collections.Generic.List[LisWirelessAccessPoint]] GetAll() {
        ...
        return [LisWirelessAccessPoint[]][LisObject]::_getAll([LisWirelessAccessPoint], [LisWirelessAccessPoint]::_getItemCommand)
    }
    static [LisWirelessAccessPoint] Get([string] $identifier) {
        if ($null -eq [LisWirelessAccessPoint]::_getItemCommand) {
            [LisWirelessAccessPoint]::_getItemCommand = Get-Command -Name Get-CsOnlineLisWirelessAccessPoint
        }
        return [LisWirelessAccessPoint][LisObject]::_get([LisWirelessAccessPoint], [LisWirelessAccessPoint]::_getItemCommand, $identifier, @{})
    }
}

class LisObjectHelper {
    static [Collections.Generic.Dictionary[string, Collections.Generic.List[LisObject]]] GetAll() {
        return [LisObjectHelper]::GetAll($false)
    }
    static [Collections.Generic.Dictionary[string, Collections.Generic.List[LisObject]]] GetAll([bool] $IncludeOrphaned) {
        $result = [Collections.Generic.Dictionary[string, Collections.Generic.List[LisObject]]]@{}
        # get the locaitons and address first to try and pull everything
        [LisObjectHelper]::LoadCache()
    ...
    static [void] LoadCache([PSFunctionHost] $parent) {
        ...
        $lCount = [LisLocation]::GetAll($true).Count
        ...
        $aCount = [LisCivicAddress]::GetAll($true).Count
        ...
        $nCount = [LisNetworkObject]::GetAll().Where({ !$_.IsOrphaned() }).Count
        ...
        $lCount1 = [LisLocation]::GetAll($true).Count
        ...
        $aCount2 = [LisCivicAddress]::GetAll($true).Count
        $cacheHelper.WriteVerbose(('Cached {0} additional LisCivicAddress objects' -f ($aCount2 - $aCount)))

        $cacheHelper.Complete()
    }
}

class LisLocationEqualityComparer : Collections.Generic.IEqualityComparer[object] {
...
class LisLocationPrioritySet : Collections.Generic.HashSet[object] {
    LisLocationPrioritySet() : base([LisLocationEqualityComparer]::new()) {}
    LisLocationPrioritySet([Collections.Generic.IEnumerable[object]] $collection) : base([LisLocationEqualityComparer]::new()) {
    ...
    [bool] Add([object] $obj) {
        ...
        if ($null -ne $existingLocation) {
            ...
            ([Collections.Generic.HashSet[object]]$this).Add($location)
            return $true
        }
        ([Collections.Generic.HashSet[object]]$this).Add($location)
        return $true
    ...
    [void] UnionWith([Collections.Generic.IEnumerable[object]] $other) {
